...
def mnist_noniid(args,dataset, num_users):
    """
    Sample non-I.I.D client data from MNIST dataset 从MNIST数据集中得到non-I.I.D的客户端样本
    :param dataset:
    :param num_users:
    :return:
    """
...
def fmnist_noniid(args,dataset, num_users):
    """
    Sample non-I.I.D client data from MNIST dataset 从FMNIST数据集中得到non-I.I.D的客户端样本
    :param dataset:
    :param num_users:
    :return:
    """
...
def cifar_noniid(args,dataset,num_users):
    ...
    # 创建一个字典idx，包含10个键值对，每个键都是一个整数，值是一个空的NumPy数组，数组的数据类型为'int64'
    idx = {i: np.array([], dtype='int64') for i in range(10) }

    j = 0
    # print((dataset[0][0]))
    for i in dataset:
        idx[i[1]] = np.append(idx[i[1]],j)  # 用np.append()函数将变量j添加到idx字典中键为i[1]的NumPy数组中
        j += 1
    ...
    # 如果k=4，则指定客户端最多只能拥有4个类的样本
    k = args.overlapping_classes
    # print(idx)
    num_examples = int(num_items/(k*num_users))  # 每个客户端的样本数量
    
    for i in range(num_users):
        t = 0
        while(t!=k):
            j = random.randint(0,9)  # 生成一个0（包含）和9（包含）之间的随机整数
